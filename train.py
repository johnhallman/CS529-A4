...
from keras.layers import Dense, Conv2D, Flatten, MaxPooling2D, Activation, Input, Dropout, \
    Add, add, LSTM, Bidirectional, Multiply, concatenate
...
def model_new_final(x, y, embedding, filepath, epochs=100, hidden_dim=512):
	...
	a2 = Input(shape=(seq_length,))
	b2 = Embedding(vocab_size, vec_dim, weights=[embedding], input_length=seq_length)(a2) # allow trainable!!!
	#b2 = LSTM(hidden_dim, activation='tanh', return_sequences=True)(b2)
...
if __name__ == "__main__":
	...
	"""
	x_small, y_small = [x_train[0][:100], x_train[1][:100]], y_train[:100] # temporary
	x_test_small, y_test_small = [x_test[0][:100], x_test[1][:100]], y_test[:100]
	model = model_one(x_small, y_small, embedding, "small-model", epochs=1)

	y_pred = model_output(model, x_test_small, i_to_a)
	score = evaluate(y_pred, y_test_small)
	print("Small score: {}".format(score))
	write_results(y_pred, "small_preds.txt")

	print(" - Small model finished training and evaluation")


	model_add = model_one(x_train, y_train, embedding, "add-model", epochs=15)
	y_pred_add= model_output(model_add, x_test, i_to_a)
	score_add = evaluate(y_pred_add, y_test)
	print("Concat score: {}".format(score_add))
	write_results(y_pred_add, "add_preds.txt")

	print(" - Add model finished training and evaluation")


	model_concat = model_one(x_train, y_train, embedding, "concat-model", epochs=15)
	y_pred_concat = model_output(model_concat, x_test, i_to_a)
	score_concat = evaluate(y_pred_concat, y_test)
	print("Concat score: {}".format(score_concat))
	write_results(y_pred_concat, "concat_preds.txt")

	print(" - Concat model finished training and evaluation")
	"""

	model_last = model_new_final(x_train, y_train, embedding, "finalfinal-model", epochs=6, hidden_dim=512)
	#y_pred_concat = model_output(model_no_embed, x_test, i_to_a)
	score_concat = ultimate_evaluate("finalfinal_preds.json", model_last, x_test, y_test, q_test, i_to_a)
	print("Final final score: {}".format(score_concat))

	print(" - Final model finished training and evaluation")
